//
// Created by Daniela Postigo on 12/31/17.
//
...
extension Itemable {
	...
	public func row(at indexPath: IndexPath) -> Row {
		return self.content[indexPath.section].rows[indexPath.item]
	}
...
extension Array where Iterator.Element == (section: Any, rows: Array<Any>) {
	...
	public subscript (row indexPath: IndexPath) -> Any {
		get {
			...
			return element.rows[indexPath.item]
		}
		set {
			var element = self[indexPath.section]
			element.rows[indexPath.item] = newValue
